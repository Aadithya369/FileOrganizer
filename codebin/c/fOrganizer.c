...
int main(){
	...
	char extension[20];
	FILE *p1 = popen("ls -1  ~/Desktop/codebin","r");	
	...
	while(fgets(read_buffer,sizeof(read_buffer),list)){
		...
		extension[j] = '\0';
		char command1[200];
		snprintf(command1,sizeof(command1),"mkdir -p ~/Desktop/codebin/%s",extension);
		system(command1);
		char command2[200];
		snprintf(command2,sizeof(command2),"mv ~/Desktop/codebin/*%s ~/Desktop/codebin/%s",extension,extension);
		system(command2);
	}
